...
// Config-unaware survivors of the old builtin.ts API. Tasks 6 and 8 migrate
// the remaining callers (commands.ts, index.ts, the TUI) and delete these.

/** @deprecated use availableCommands(config) — this ignores the toggles. */
export function builtinCommands(): ExtensionCommand[] {
  return allExtensions().flatMap((e) => e.commands ?? []);
}

/** @deprecated use findCommand(config, name). */
export function findBuiltinCommand(name: string): ExtensionCommand | undefined {
  return builtinCommands().find((c) => c.name === name);
}

/** @deprecated use runCommand(config, …). */
...
/** @deprecated use startupExtensions. */
export const startupBuiltins = startupExtensions;

/** @deprecated use listExtensions(config). */
